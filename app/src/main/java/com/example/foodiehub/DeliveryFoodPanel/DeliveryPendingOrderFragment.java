...
import com.example.foodiehub.MainMenu;
...
public class DeliveryPendingOrderFragment extends Fragment {
    ...
    String deliveryId = "YtqRJYv3Idf8yJDA9r98WF81TBG2";

    @Nullable
    @Override
    public View onCreateView(@NonNull LayoutInflater inflater, @Nullable ViewGroup container, @Nullable Bundle savedInstanceState) {
        @SuppressLint("InflateParams") final View view = inflater.inflate(R.layout.fragment_deliverypendingorder, null);
        requireActivity().setTitle("Pending Orders");
    ...
    private void DeliveryPendingOrders() {
        ...
        DatabaseReference databaseReference = FirebaseDatabase.getInstance().getReference("DeliveryShipOrders").child(deliveryId);
            ...
            public void onDataChange(@NonNull DataSnapshot dataSnapshot) {
                if (dataSnapshot.exists()) {
                    ...
                    for (DataSnapshot snapshot : dataSnapshot.getChildren()) {
                        DatabaseReference data = FirebaseDatabase.getInstance().getReference("DeliveryShipOrders").child(Objects.requireNonNull(FirebaseAuth.getInstance().getCurrentUser()).getUid()).child(Objects.requireNonNull(snapshot.getKey())).child("OtherInformation");
